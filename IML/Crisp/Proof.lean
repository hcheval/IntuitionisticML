...
/-!
# The original proof system (archived)

This is the proof system as it accompanied the paper, preserved verbatim under
the namespace `IML.Crisp` (only the namespace changed). Its SINGLETON rule is
the classical negative form `~(C₁[x ⊓ φ] ⊓ C₂[x ⊓ ~φ])`. The archived
soundness theorem `IML.Crisp.soundness` is stated over this file, so the
archive is independent of any later change to the current system `IML.Proof`;
`IML.Crisp.Bridge` embeds every derivation of this system into the current one
(`Proof.toGeneral`).
-/
