...
/-!
# The original crisp-equality development (archived)

This is the original development of the Heyting algebra semantics, in which a
model interprets element variables with ordinary decidable equality:
`⟦x⟧(m) = if m = ρ(x) then ⊤ else ⊥`. It is preserved here as published, under
the namespace `IML.Crisp`, with only the namespace and the internal imports
changed. The generalization to `L`-valued equality lives in
`IML.HeytingSemantics`, and the crisp case is recovered there as an instance in
`IML.CrispModels`; `IML.Crisp.Bridge` proves that the two agree on crisp models.
-/
